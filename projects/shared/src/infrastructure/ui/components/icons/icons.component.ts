...
export class IconsComponent implements OnChanges {
  ...
  // Este sería para pasar directamente un SVG personalizado
  @Input() svgContent: string | null = null;

  public iconSvg: SafeHtml | null = null;

  constructor(private sanitizer: DomSanitizer) {}

  ngOnChanges(changes: SimpleChanges): void {
    // Priorizar SVG personalizado si se proporciona
    if (this.svgContent) {
      this.iconSvg = this.sanitizeSvg(this.setIconAttributes(this.svgContent));
    } else if (this.name) {
      // En una implementación real, aquí obtendríamos el SVG de lucide o similar
      // Por ahora, usamos un mapa simple con algunos iconos comunes
      const iconSvg = this.getIconByName(this.name);
      if (iconSvg) {
        this.iconSvg = this.sanitizeSvg(this.setIconAttributes(iconSvg));
      }
    }
  }
  ...
  private setIconAttributes(svg: string): string {
    // Implementar un color dinámico para el SVG
    return svg.replace('<svg', `<svg fill="${this.color}"`);
  }

  private getIconByName(name: string): string {
    // Mapa simplificado de algunos iconos comunes de la interfaz
    // En una implementación real, se integraría con una biblioteca como lucide-angular
